...
jQuery(document).ready(function ($) {
    ...
    autoCompleteAccount($acctLkupFrom, $account2Id, accountSelected);
    autoCompleteItem();
    bindLinesEvents();
    getcurrencyrate();
    $('#sub_total').val('0');
...
function bindLineEvents($tr) {
    ...
    function calculateLineTotal() {
        ...
        jQuery('td.i-total', $tr).text(total);
    }
...
function validation_order_to_invoice() {
    var count = 0;
    var test = [];
    jQuery('tr', $transactionLines).each(function (i, tr) {
        ...
        if (shelf === "0") {
            document.getElementById('error_transactionLines').innerHTML = "*Please select a shelf*";
