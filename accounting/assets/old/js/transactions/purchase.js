...
function validation() {
	...
	jQuery('tr', $transactionLines).each(function (i, tr) {
		...
		if (jQuery('input.i-item_qty', tr).val() != 0 && sizes_qty != 0) {
			document.getElementById('error_transactionLines').innerHTML = "*Enter qty field only or qty for each size but not both*";
			document.getElementById('error_transactionLines').className = "alert alert-danger";
			count++;
		}
	...
	var op_nb = $('#op_nb').val()
	if (op_nb.trim() == "") {
		document.getElementById('error_op_nb').innerHTML = "*please enter number for OP nb*";
		document.getElementById('error_op_nb').className = "alert alert-danger";
		count++;
	} else {
		if (isNaN(op_nb) == true) {
			document.getElementById('error_op_nb').innerHTML = "*OP nb must be numeric*";
			document.getElementById('error_op_nb').className = "alert alert-danger";
			count++;
		} else {
			if (Math.sign(op_nb) == -1) {
				document.getElementById('error_op_nb').innerHTML = "*OP nb must be positive number*";
				document.getElementById('error_op_nb').className = "alert alert-danger";
				count++;
			} else {
				...
				$.ajax({
					...
					success: function (data) {
						if (data !== false) {
							document.getElementById('error_op_nb').innerHTML = "*Must be unique*";
							document.getElementById('error_op_nb').className = "alert alert-danger";
							count++;
						}
					}
