...
function validation() {
    ...
    if (document.accountForm.phone.value == "") {
        ...
        if ($('#phone').val().trim() == "+961") {
            document.getElementById('error_phone').innerHTML = _lang.Please_enter_mobile_number;
            document.getElementById('error_phone').className = "alert alert-danger";
            count++;
        } else {
            let phone_rep = $('#phone').val().trim().replace(" ", "");
            let first4 = phone_rep.slice(0, 4); // +961
            if (first4 == '+961') {
                let phone_nbs = phone_rep.replace("+961", "");
                if (parseInt(phone_nbs.length) < 7) {
                    document.getElementById('error_phone').innerHTML = _lang.mobile_number_at_least_7_after_code;
                    document.getElementById('error_phone').className = "alert alert-danger";
                    count++;
                }
            } else {
                document.getElementById('error_phone').innerHTML = _lang.mobile_number_start_961;
                document.getElementById('error_phone').className = "alert alert-danger";
                count++;
            }
