jQuery(document).ready(function () {
	window.returnpurchasesTblDT = null;
	let dtActionsHTML = '<a href="' + getAppURL('return_purchases/edit/') + '%d" class="btn bt-link btn-xs" title="Edit"><i style="color:#282828;" class="glyphicon glyphicon-pencil"></i></a>' +
			'<a href="javascript:void(0);" onclick="confirmDelete(\'' + getAppURL('return_purchases/delete/') + '%d\')"class="btn bt-link btn-xs" title="Delete" >' +
			'<i class="glyphicon glyphicon-trash text-danger"></i></a>';
	/***************************************************/
	var $dtTbl = $('#returnpurchasesTbl');
	EnhanceDataTableSearch(window.returnpurchasesTblDT = $dtTbl.DataTable({
		...
		order: [[0, 'asc']], ajax: {url: getAppURL('return_purchases/index'), type: 'GET', searchDelay: _GST},
