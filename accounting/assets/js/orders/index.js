jQuery(document).ready(function () {
	window.salesDT = null;
	let dtActionsHTML = '<a href="' + getAppURL('orders/edit/') + '%d" class="btn bt-link btn-xs" title="Edit"><i style="color:#282828;" class="glyphicon glyphicon-pencil"></i></a>' +
		'<a href="javascript:void(0);" onclick="confirmDelete(\'' + getAppURL('orders/delete/') + '%d\')"class="btn bt-link btn-xs" title="Delete" >' +
		'<i class="glyphicon glyphicon-trash text-danger"></i></a>' +
		'<a href="' + getAppURL('orders/missing_products/') + '%d" class="btn bt-link btn-xs" title="Missing Products"><i style="color:#282828;" class="glyphicon glyphicon-edit"></i></a>' +
		'<a href="' + getAppURL('orders/to_invoice/') + '%d" class="btn bt-link btn-xs" title="To Invoice"><i style="color:#282828;" class="glyphicon glyphicon-check"></i></a>';
	...
	EnhanceDataTableSearch(window.salesDT = $dtTbl.DataTable({
		...
		order: [[0, 'asc']], ajax: { url: getAppURL('orders/index'), type: 'GET', searchDelay: _GST },
