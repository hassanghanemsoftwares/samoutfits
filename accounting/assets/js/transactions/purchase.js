...
jQuery(document).ready(function ($) {
  ...
  if (
    window.location.href == getAppURL("purchases/add") ||
    window.location.href == getAppURL("order_purchases/add")
  ) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.ajax({
            ...
            url: getAppURL(
              "purchases/get_purchases_account_with_the_same_currency"
            ),
            success: function (data) {
              $("#acctLkupFrom").val(
...
function addLineToTransaction(item, newLine) {
  ...
  if (newLine) {
    ...
    if (
      jQuery("select.i-warehouse", $tr).find(
        'option[value="Primary Warehouse"]'
      ).length > 0
    ) {
...
function bindLineEvents($tr) {
  ...
  function calculateLineTotal() {
    let net_cost =
      parseFloat($price.val()) * (1 - $discount.val() / 100) +
      parseFloat($cost.val());
...
function calculateSubtotalAndTotal($total_discount) {
  ...
  $("#final_total").val(
    (subtot - parseFloat($total_discount.val())).toFixed(2)
  );
}
    ...
    if (Math.sign(op_nb) == -1) {
      ...
      if (op_nb.trim() != "") {
        $.ajax({
          ...
          success: function (data) {
            if (data !== false) {
              document.getElementById("error_op_nb").innerHTML =
                _lang.unique_value;
              document.getElementById("error_op_nb").className =
                "alert alert-danger";
              count++;
            }
          },
...
function autoCompletePurchasesAccount(
  $acctLkup,
  $accountId,
  onResultSelection,
  $currency_id
  ...
  $acctLkup.autocomplete({
    serviceUrl: getAppURL(
      "purchases/lookup_purchases_accounts/".concat($currency_id)
    ),
    appendTo: $acctLkup.parent()[0],
    noCache: true,
    showNoSuggestionNotice: true,
