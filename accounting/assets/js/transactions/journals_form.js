...
function autoCompleteAccount($acctLkup, $accountId, onResultSelection) {
	console.log($acctLkup, $accountId, onResultSelection)
...
function validation() {
	...
	if (amount.trim() == "") {
		document.getElementById('error_amount').innerHTML = _lang.field_required;
		document.getElementById('error_amount').className = "alert alert-danger";
		count++;
	}
	if (isNaN(amount) == true) {
		document.getElementById('error_amount').innerHTML = _lang.numeric_nb;
		document.getElementById('error_amount').className = "alert alert-danger";
		count++;
	} else {
		if (Math.sign(amount) == -1) {
			document.getElementById('error_amount').innerHTML = _lang.positive_nb;
			document.getElementById('error_amount').className = "alert alert-danger";
			count++;
		}
	}
