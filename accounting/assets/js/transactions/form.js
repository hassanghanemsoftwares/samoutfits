		...
		$.ajax({
			...
			success: function (data) {
				...
				if (uniqueData.length > 1) {
					jQuery('select.i-warehouse', $tr).append(`<option value=""></option>`);
				}
...
function validation() {
	...
	jQuery('tr', $transactionLines).each(function (i, tr) {
		...
		if (isNaN(qty) == true || isNaN(price) == true || isNaN(discount) == true) {
			...
			if (Math.sign(qty) == -1 || Math.sign(price) == -1 || Math.sign(discount) == -1) {
				document.getElementById('error_transactionLines').innerHTML = _lang.positive_valuess;
				document.getElementById('error_transactionLines').className = "alert alert-danger";
				count++;
			}
		}
		...
		if(warehouse_row == null || warehouse_row.trim() == ""){			
			let text_w_msg = '<p style="font-size: 9px; font-weight: bold;" class="text-danger error-warehouse_row">Required!</p>';
		...
		if(shelf_row == null || shelf_row.trim() == ""){			
			let text_w_msg = '<p style="font-size: 9px; font-weight: bold;" class="text-danger error-shelf_row">Required!</p>';
			jQuery('select.i-shelf', tr).parent().append(text_w_msg);
			jQuery('select.i-shelf', tr).css('border-color', '#e74c3c');
			count++;
		}
		let row = { row_id: $(this).attr('id'), item_id: row_item_id, warehouse: warehouse_row, shelf: shelf_row, qty: qty, size: size_row };
		records[c] = row;
		c++;
	...
	$.ajax({
		...
		type: 'POST',
		async: false,
		data: { 'records': records, trans_id: $('#id').val(), branch_id: $('#branches').val() },
		...
		success: function (errors) {
			if (errors.length > 0) {
				...
				$.each(errors, function (index, error) {
					$('#' + error.row_id).find('td input.i-qty').css('border-color', '#e74c3c');
					let text_msg = '<p style="font-size: 9px; font-weight: bold;" class="text-danger error-qty_validation">Available Qty = ' + error.av_qty + '</p>';
					$('#' + error.row_id).find('td input.i-qty').parent().append(text_msg);
				});
			}
		}
	});
