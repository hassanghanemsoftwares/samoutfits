  ...
  if (newLine) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var getselected = jQuery("select.i-from", $tr)
          ...
          success: function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
              jQuery("select.i-sizes", $tr)
                .append(`<option value="${data[i].size}">
                                        ${data[i].size} </option>`);
            }
            $.ajax({
              cache: false,
              ...
              data: {
                ...
                item_id: item.id,
                size: jQuery("select.i-sizes", $tr).val(),
              },
              url: getAppURL(
                "warehouses/get_item_max_qty_by_warehouse_id_and_size"
              ),
              success: function (data) {
...
function validation() {
  ...
  jQuery("tr", $transactionLines).each(function (i, tr) {
    ...
    if (jQuery("select.i-from", tr).val() == jQuery("select.i-to", tr).val()) {
      document.getElementById("error_transactionLines").innerHTML =
        _lang.please_select_w_s_different_than_from;
      document.getElementById("error_transactionLines").className =
        "alert alert-danger";
      count++;
    }
