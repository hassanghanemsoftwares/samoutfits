  ...
  $("#bulk").on("click", function (e) {
    ...
    $("table > tbody > tr").each(function () {
      rows[count] = {
        ...
        "Payment method": $(this).find("td").eq(8).text(),
        "Payment Status": $(this).find("td").eq(9).text(),
        subtotal: $(this).find("td").eq(10).text(),
        "delivery charge": $(this).find("td").eq(11).text(),
        total: $(this).find("td").eq(12).text(),
        qty: $(this).find("td").eq(13).text(),
        status: $(this).find("td").eq(14).text(),
        "dispatch date": $(this).find("td").eq(15).text(),
        User: $(this).find("td").eq(17).text(),
      };
      ids[count] = { id: $(this).find("td").eq(18).find("input").val() };
