jQuery(document).ready(function () {
  ...
  $("#bgsave").on("click", function (e) {
    if ($("table > tbody > tr").length != 0) {
      ...
      if (tot != 0) {
        alert(
          "Please, Check and Edit your Excel sheet before submit because it still have errors."
        );
      } else {
        if (validation() == true) {
          var auto_nbs = [];
          ...
          var count = 0;
          $("table > tbody > tr").each(function () {
            auto_nbs[count] = $(this).find("td").eq(1).text();
            amounts[count] = $(this).find("td").eq(3).text();
            count++;
          });
          $.ajax({
            cache: false,
...
function ExportToExcel(type, fn, dl) {
  ...
  // Generate an array of column widths (25 for each column)
  ...
  return dl
    ...
    : XLSX.writeFile(
        wb,
        fn || "receive_cash_" + today + "." + (type || "xlsx")
      );
}
...
function validation() {
  ...
  if ($("#cash_date").val() == "" || isNaN(date1.getDate())) {
    ...
    document.getElementById("error_cash_date").innerHTML =
      "*Please enter a valide date*";
