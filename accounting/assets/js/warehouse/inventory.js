jQuery(document).ready(function () {
  ...
  let dtActionsHTML =
      '<a href="' +
    getAppURL("barcodes/generate_inventory/") +
    '%d/%x" class="btn bt-link btn-xs" title="Generate Barcode"><i style="color:#686868;" class="glyphicon glyphicon-print"></i></a>' +
  ...
  EnhanceDataTableSearch(
    (window.warehousesDT = $dtTbl.DataTable({
      orderCellsTop: true,
      ...
      columnDefs: [
        {
          targets: 8,
          orderable: false,
          createdCell: (td, itemId, row) => {
            $(td)
              .addClass("text-right")
              .html(
                dtActionsHTML
                  .replace(/%d/g, parseInt(itemId))
...
function transfervalidate() {
  ...
  if (current_ws == new_ws) {
    document.getElementById("error_transfer_new_warehouse").innerHTML =
      _lang.select_different_warehouse_than_the_current_warehouse;
    document.getElementById("error_transfer_new_warehouse").className =
      "alert alert-danger";
    count++;
  }
  if (count == 0) {
