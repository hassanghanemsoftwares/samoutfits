jQuery(document).ready(function () {
  ...
  let dtActionsHTML =
    ...
    getAppURL("items/delete/") +
    '%d\')"class="btn bt-link btn-xs" title="Delete" >' +
    '<i class="glyphicon glyphicon-trash text-danger"></i></a>' +
    '<a href="' +
    getAppURL("barcodes/generate/") +
    '%d" class="btn bt-link btn-xs" title="Generate Barcode"><i style="color:#686868;" class="glyphicon glyphicon-print"></i></a>' +
    '<a href="' +
    getAppURL("items/activity/") +
    '%d" class="btn bt-link btn-xs" title="Activity"><i class="glyphicon glyphicon-info-sign text-danger"></i></a>' +
    '<a href="' +
    getAppURL("items/warehouses/") +
    '%d" class="btn bt-link btn-xs" title="Warehouses"><i style="color:#505050;" class="glyphicon glyphicon-th"></i></a>' +
    '<a href="' +
    getAppURL("items/add_opening_item/") +
    '%d" class="btn bt-link btn-xs" title="Opening Item"><i class="glyphicon glyphicon-open text-danger"></i></a>' +
    '<input type="text" value="%d" hidden>';
  /***************************************************/
  var $dtTbl = $("#itemsTbl");
  BuildDataTableColumnSearch($dtTbl, "itemsDT");
  EnhanceDataTableSearch(
    (window.itemsDT = $dtTbl.DataTable({
      orderCellsTop: true,
      ...
      columnDefs: [
        {
          targets: 10,
          orderable: false,
          createdCell: (td, accId) =>
            $(td)
              .addClass("text-right")
              .html(dtActionsHTML.replace(/%d/g, parseInt(accId))),
        },
      ],
      stateSave: true,
  ...
  $("#bulk_submit").on("click", function (e) {
    ...
    if (edit_stock_clearance === true) {
      $.ajax({
        ...
        data: {
          ids: ids,
          stock_clearance: $("#stock_clearance").val(),
        },
    ...
    if (edit_cool_storage === true) {
      $.ajax({
        ...
        data: {
          ids: ids,
          cool_storage: $("#cool_storage").val(),
        },
        url: getAppURL("items/bulk_product_cool_storage_edit"),
        success: function (data) {
          // console.log(data);
          // return false;
          // setTimeout(() => $('#bulk_modal').modal('hide'), 1000);
    ...
    if (edit_flammable_handling === true) {
      $.ajax({
        cache: false,
        type: "POST",
        async: true,
        data: {
          ids: ids,
          flammable_handling: $("#flammable_handling").val(),
        },
        url: getAppURL("items/bulk_product_flammable_handling_edit"),
        success: function (data) {
          // console.log(data);
          // return false;
          // setTimeout(() => $('#bulk_modal').modal('hide'), 1000);
...
function export_pixcel_excel() {
  $("#export_pixcel_excel").html(
    '<span class="text-white"><span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Loading...</span>'
  );
  ...
  $.ajax({
    ...
    success: function (jsonData) {
      ...
      $("#export_pixcel_excel").html(
        '<i class="glyphicon glyphicon-save-file" style="margin-right: 10px;"></i>' +
          _lang.Pixel_Excel
      );
