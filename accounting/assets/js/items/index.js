jQuery(document).ready(function () {
	...
	let dtActionsHTML = '<a href="' + getAppURL('items/edit/') + '%d" class="btn bt-link btn-xs" title="Edit"><i style="color:#282828;" class="glyphicon glyphicon-pencil"></i></a>' +
		'<a href="javascript:void(0);" onclick="confirmDelete(\'' + getAppURL('items/delete/') + '%d\')"class="btn bt-link btn-xs" title="Delete" >' +
		'<i class="glyphicon glyphicon-trash text-danger"></i></a>' +
		'<a href="' + getAppURL('barcodes/generate/') + '%d" class="btn bt-link btn-xs" title="Generate Barcode"><i style="color:#686868;" class="glyphicon glyphicon-print"></i></a>'
		+ '<a href="' + getAppURL('items/activity/') + '%d" class="btn bt-link btn-xs" title="Activity"><i class="glyphicon glyphicon-info-sign text-danger"></i></a>'
		+ '<a href="' + getAppURL('items/warehouses/') + '%d" class="btn bt-link btn-xs" title="Warehouses"><i style="color:#505050;" class="glyphicon glyphicon-th"></i></a>'
		+ '<a href="' + getAppURL('items/add_opening_item/') + '%d" class="btn bt-link btn-xs" title="Opening Item"><i class="glyphicon glyphicon-open text-danger"></i></a>' +
	...
	EnhanceDataTableSearch(window.itemsDT = $dtTbl.DataTable({
		...
		columns: [{ data: 'barcode' }, { data: 'description' }, { data: 'category' }, { data: 'cost' },
		{ data: 'purchase_cost' }, { data: 'profit' }, { data: 'price' }, { data: 'price_ttc' }, { data: 'qty' }, { data: 'published' }, { data: 'id' }
		],
		columnDefs: [
	...
	$('#bulk').on('click', function (e) {
		...
		$("table > tbody > tr").each(function () {
			rows[count] =
			{
				...
				'Sale Price': $(this).find('td').eq(6).text(),
				'Gross Sale Price': $(this).find('td').eq(7).text()
			};
			ids[count] = { 'id': $(this).find("td").eq(10).find("input").val() };
	...
	$('#bulk_submit').on('click', function (e) {
		...
		var edit_variant1 = $('#edit_variant1').is(':checked');
		...
		var tags = $('#tags').val();
		var cost = $('#cost').val();
		if (edit_price === true || edit_cost === true || edit_publish === true || edit_stock_clearance === true || edit_category == true || edit_sub_category == true || edit_variant1 == true || edit_color == true || edit_gender == true || edit_old_price == true || edit_tags == true) {
			var count = -1;
			var ids = [];
		...
		if (edit_variant1 === true) {
			$.ajax({
				cache: false,
				type: 'POST',
				async: true,
				data: {
					'ids': ids, 'variant1': variant1
				},
				url: getAppURL('items/bulk_product_variant1_edit'),
				success: function (data) {
					location.reload();
				}
			});
	...
	$('#reset_filters').on('click', function (e) {
		...
		$('#gender_filter').val('');
		$('#publish_filter').val('');
		$('#stock_clearance_filter').val('');
		$('#tag_barcodes').clearAllTags();
		$('#itemsTbl thead').find("tr:eq(1)").find('input').each(function (n) {
	...
	$("#export_pixcel_excel").on("click",function(){
		export_pixcel_excel();
	});
});
...
function export_pixcel_excel(){
	$("#export_pixcel_excel").html('<span class="text-white"><span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Loading...</span>');
	$("#export_pixcel_excel").attr('disabled', 'disabled');
	$.ajax({
		cache: false,
		type: 'POST',
		async: true,
		data: {},
		url: getAppURL('items/generate_pixel_excel'),
		success: function (jsonData) {
			let items_count = parseInt(jsonData.count);
            // console.log(items_count)
            // Create a new workbook and add a worksheet
            var workbook = new ExcelJS.Workbook();
            var worksheet = workbook.addWorksheet('Sheet1');

            worksheet.addRows(jsonData.data);
            ...
            worksheet.getColumn("E").width = 100;
            worksheet.getColumn("F").width = 100;
            worksheet.getColumn("G").width = 100;
            worksheet.getColumn("H").width = 100;
            worksheet.getColumn("I").width = 100;
            worksheet.getColumn("J").width = 100;
            worksheet.getColumn("K").width = 100;
            worksheet.getColumn("L").width = 100;
            worksheet.getColumn("M").width = 100;
            worksheet.getColumn("N").width = 100;
            worksheet.getColumn("O").width = 100;
            worksheet.getColumn("P").width = 100;
            worksheet.getColumn("Q").width = 100;
            worksheet.getColumn("R").width = 100;
            worksheet.getColumn("S").width = 100;
            worksheet.getColumn("T").width = 100;
            worksheet.getColumn("U").width = 100;
            worksheet.getColumn("V").width = 100;
            worksheet.getColumn("W").width = 100;
            worksheet.getColumn("X").width = 100;
			worksheet.getColumn("Y").width = 100;
			worksheet.getColumn("Z").width = 100;
			worksheet.getColumn("AA").width = 100;
			worksheet.getColumn("AB").width = 100;
			worksheet.getColumn("AC").width = 100;
            ...
            workbook.xlsx.writeBuffer().then(function (buffer) {
                ...
                link.download = 'Pixel_Items.xlsx';
                link.click();
            });
            $("#export_pixcel_excel").html('<i class="glyphicon glyphicon-save-file" style="margin-right: 10px;"></i>'+_lang.Pixel_Excel);
            $("#export_pixcel_excel").prop('disabled', false);
		}
	});
}