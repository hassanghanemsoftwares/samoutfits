...
function autoCompleteAccountSearch($acctLkup) {
  $acctLkup.autocomplete({
    serviceUrl: function () {
      const inputVal = $acctLkup.val();
      if (inputVal.charAt(0) === "[") {
        return getAppURL("sales/lookup_seatch_transactions");
      } else {
        return getAppURL("accounts/lookup_seatch_accounts");
      }
    },
    ...
    onSelect: function (result) {
      window.location.href =
        "reports/accounts_history/?account_id=" + result.row.id;
    },
