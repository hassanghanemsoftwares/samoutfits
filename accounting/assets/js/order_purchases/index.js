jQuery(document).ready(function () {
	...
	let dtActionsHTML = '<a href="' + getAppURL('order_purchases/edit/') + '%d" class="btn bt-link btn-xs" title="Edit"><i style="color:#282828;" class="glyphicon glyphicon-pencil"></i></a>' +
		'<a href="javascript:void(0);" onclick="confirmDelete(\'' + getAppURL('order_purchases/delete/') + '%d\')"class="btn bt-link btn-xs" title="Delete" >' +
		'<i class="glyphicon glyphicon-trash text-danger"></i></a>' +
		'<a href="' + getAppURL('order_purchases/to_purchase/') + '%d" class="btn bt-link btn-xs" title="Add to Order"><i style="color:#282828;" class="glyphicon glyphicon-check" ></i></a>'+
	...
	EnhanceDataTableSearch(window.purchasesDT = $dtTbl.DataTable({
		...
		order: [[0, 'asc']], ajax: { url: getAppURL('order_purchases/index'), type: 'GET', searchDelay: _GST },
