<?php foreach ($records as $x => $record) : ?>
    <tr id="<?php echo 'item-', $x ?>" data-index="<?php echo $x ?>">
        ...
        <td><?php echo form_dropdown('transItems[' . $x . '][warehouses]', $warehouses_list, $warehouse[$x], 'id="warehouses" class="form-control i-warehouse" style="width: 100px;"')
            ?></td>
        <td><?php echo form_dropdown('transItems[' . $x . '][shelfs]', $shelf_list[$x], $shelf[$x], 'id="shelfs" class="form-control i-shelf" style="width: 100px;"')
            ?></td>
        <td><input name="transItems[<?php echo $x ?>][item_qty]" value="<?php echo $record['item_qty'] ?>" id="item_qty" type="text" class="form-control i-item_qty" style="width: 70px;" /></td>
        <?php if ($sizes[0] !== '') {
            foreach ($sizes as $s) { ?>
