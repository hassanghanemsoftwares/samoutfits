<?php foreach ($transItems as $x => $transItem) : ?>
    <tr id="<?php echo 'item-', $x ?>" data-index="<?php echo $x ?>">
        ...
        <td><?php echo form_dropdown('transItems[' . $x . '][warehouses]', $warehouses[$x], $warehouse[$x],  'id="warehouses" class="form-control i-warehouse"')
            ?></td>
        <td><?php echo form_dropdown('transItems[' . $x . '][shelfs]', $shelfs[$x], $shelf[$x], 'id="shelfs" class="form-control i-shelf"')
            ?></td>
        <td><?php echo form_dropdown('transItems[' . $x . '][sizes]', $item_sizes[$x], $size[$x], 'id="sizes" class="form-control i-sizes" style="width: 100px;"')
            ?></td>
        <td><input name="transItems[<?php echo $x ?>][qty]" value="<?php echo $transItem['qty'] ?>" id="qty" type="text" class="form-control i-qty" />
