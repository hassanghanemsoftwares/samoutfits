 ...
 * @property Journal_account $Journal_account
 */
class Payments extends MY_Controller
{
	...
	{
		parent::__construct();
		// $this->pageTitle = $this->lang->line('payments');
	...
	public function index()
	{
		if ($this->input->is_ajax_request()) {
			$this->_render_json($this->Journal->load_journals_data_tables(Journal::PaymentJournalType));
		} else {
			// $this->pageTitle = $this->lang->line('payments');
			$data['records'] = $this->Journal->paginate_payment();
			...
			$this->load->view('templates/footer', ['_moreJs' => ['jquery.dataTables.min', 'dataTables.bootstrap.min', 'dataTables.datetime.format', 'payments/index']]);
		}
	}

	public function add()
	{
		$this->save(0, Journal::PaymentJournalType);
	}

	public function edit($id = '0')
	{
		$this->save($id, Journal::PaymentJournalType);
	}
		...
		if ($post['submitBtn']) {
			...
			if ($saved) {
				if (!$fetched) {
					...
					$this->Journal->save_in_journal_accounts($journal_id["0"]["id"], $post['trans']['account_id'], $post['trans']['auto_no'], $post['trans']['amount'], "-1", $name2["0"]["account_name"], "Payment");
					$this->Journal->save_in_journal_accounts($journal_id["0"]["id"], $post['trans']['account2_id'], $post['trans']['auto_no'], $post['trans']['amount'], "1", $name1["0"]["account_name"], "Payment");
					...
					$name1 = $this->Account->fetch_account_name_by_id($post['trans']['account_id']);
					$name2 = $this->Account->fetch_account_name_by_id($post['trans']['account2_id']);
					$this->Journal->save_in_journal_accounts($this->Journal->get_field('id'), $post['trans']['account_id'], $post['trans']['auto_no'], $post['trans']['amount'], "-1", $name2["0"]["account_name"], "Payment");
					$this->Journal->save_in_journal_accounts($this->Journal->get_field('id'), $post['trans']['account2_id'], $post['trans']['auto_no'], $post['trans']['amount'], "1", $name1["0"]["account_name"], "Payment");
					//update balance debit credit for account 1
					$balance = $this->Journal->calculate_account_balance($post['trans']['account_id'])["total"];
					$credit = $this->Journal->calculate_account_credit($post['trans']['account_id'])["total"];
					$debit = $this->Journal->calculate_account_debit($post['trans']['account_id'])["total"];
	...
	public function lookup_accounts_with_same_cuurency($currency_id)
	{
		$this->load->model('Account');
		$this->_render_json(
			$this->Account->search_suggestions_same_currency(trim($this->input->get('query', true)), $currency_id)
		);
	}
	...
	public function preview($id){
		...
		$this->load->view('payment/preview', $data);
		$this->load->view('templates/footer', [
			'_moreJs' => [
				'air-datepicker/js/datepicker.min', 'air-datepicker/js/i18n/datepicker.en',
