jQuery(document).ready(function () {
    $("#show_hide_password_btn").on('click', function (event) {
        event.preventDefault();
        if ($('#password').attr("type") == "text") {
            $('#password').attr('type', 'password');
            $('#show_hide_password_i').addClass("fa-eye-slash");
            $('#show_hide_password_i').removeClass("fa-eye");
        } else if ($('#password').attr("type") == "password") {
            $('#password').attr('type', 'text');
            $('#show_hide_password_i').removeClass("fa-eye-slash");
            $('#show_hide_password_i').addClass("fa-eye");
        }
    });
});
...
if ($('#phone').val() != "") {
    var phone_error_value = $('#phone').val().replace("+", "");

    var phonecountry_error_value = getCountryCode(phone_error_value)[0];
    var phonecode_error_value = getCountryCode(phone_error_value)[1];
    $('#country').val(phonecode_error_value);
    phone_error_value = phone_error_value.replace(phonecode_error_value, "")

    $('#phone_view').val(phone_error_value);
}
...
$('#phone').val("+" + $('#country').val() + $('#phone_view').val());

$('#country').on("change", function () {
    $('#phone').val("+" + $('#country').val() + $('#phone_view').val());
})

$('#phone_view').on("change", function () {
    $('#phone').val("+" + $('#country').val() + $('#phone_view').val());
})
    ...
    if ($('#phone_view').val() == '') {
        ...
        if (Array.isArray(lenthofcountry)) {
            ...
            lenthofcountry.forEach(element => {
                ...
                if ($('#phone_view').val().length == element) {
                    coutcountry = 0;
            ...
            coutcountry = 1
            if ($('#phone_view').val().length == lenthofcountry) {
                coutcountry = 0
            }
        }
        ...
        if (coutcountry != 0) {
            count = 6;
            $('#error_phone').text('Please check phone number length');
