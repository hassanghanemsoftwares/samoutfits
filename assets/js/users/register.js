jQuery(document).ready(function () {
    ...
    $('#password, #confirm_password').on('keyup', function () {
        ...
        if ($('#password').val() !== '' && $('#confirm_password').val() !== '') {
            if ($('#password').val() == $('#confirm_password').val()) {
                $('#msg_password').html('<i class="fa fa-check"></i> ' + _lang.Password_Matching).css('color', 'green');
                // $count = 0;
            } else {
                $('#msg_password').html('<i class="fa fa-exclamation"></i> ' + _lang.Password_Not_Matching).css('color', 'red');
                // $count = 1;
            }
        } else {
            $('#msg_password').html('')
    ...
    $('#country_id').on('change', function () {
        document.getElementById('country_val').value = $('#country_id').val();
    });
    $("#registerBtn").on("click",function(){
        if (validation()) {
            // $.ajax({
            //     url: getAppURL('users/send_otp'),
            //     type: "POST",
            //     data: { phone: "+"+$('#code').val() + $("#phone").val() },
            //     dataType: "json",
            //     success: function (dataResult) {
            //         if (dataResult.result) {
            //             $("#phoneOtp").text("+"+$('#code').val() + $("#phone").val());
            //             $('#otp_modal').modal('show');
            //         } else {
            //             $("#error_message_otp").text(dataResult.error);
            //             $("#place_order_without_otp_btn").prop('hidden', true);
    ...
    $("#verify_code_btn").on("click", function () {
        $.ajax({
            url: getAppURL('users/verify_opt'),
            type: "POST",
...
function validation() {
    // alert($('#msg_password').text())
    var count = 0;
    let text = $('#msg_password').text();
    // if (text.includes(_lang.Password_Matching) == false) {
    //     count++;
    // }
    if ($('#password').val() == '') {
        $('#msg_password').html(_lang.field_required).css('color', 'red');
        count++;
    } else {
        $('#msg_password').html('');
        if ($('#password').val().length < 8) {
            $('#msg_password1').html("Must be at least 8 digits").css('color', 'red');
            count++;
        } else {
            $('#msg_password1').html('');
        }
    ...
    if ($('#phone').val() == '') {
        $('#msg_phone').html(_lang.field_required).css('color', 'red');
        count++;
    } else {
        var coutcountry = 0;
        var coutcountry = 0;
        ...
        if (coutcountry != 0) {
            count = 6;
            $('#msg_phone').text('Please check phone number length').css('color', 'red');
        ...
        $.ajax({
            ...
            async: false,
            data: {
                'phone': "+" + $('#code').val() + $('#phone').val(),
                type: 0
            },
            url: getAppURL('Users/check_phone'),
        ...
        if (exitstphone == 1) {
            count = 6;
            $('#msg_phone').text('This user is already registered.').css('color', 'red');
        }
    ...
    if ($('#agree_terms').is(":checked") == false) {
        $('#msg_terms').html(_lang.field_required).css('color', 'red');
        count++;
    } else {
        $('#msg_terms').html('');
    }
