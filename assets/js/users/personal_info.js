jQuery(document).ready(function () {
    $("#email").on('keyup', function (event) {
        ...
        if ($("#email").val().match(email_format)) {
            ...
            $('#msg_email').html(_lang.Wrong_Format).css('color', 'red');
        }
    });
    $("#phone2").on('keyup', function (event) {
        ...
        if ($("#phone2").val().match(phonenb)) {
            ...
            $('#msg_phone2').html(_lang.Wrong_Format).css('color', 'red');
        }
    });
    get_city_data();
...
function validation() {
    ...
    if ($("#birthdate").val() !== '') {
        ...
        if (getAge($("#birthdate").val()) < 18) {
            $('#msg_birthdate').html("*Customer age must be 18+ years*").css('color', 'red');
            count++;
        } else {
            $('#msg_birthdate').html('');
        }
        ...
        $('#msg_birthdate').html("*" + _lang.Required + "*").css('color', 'red');
        count++;
    }
    if ($('#phone2').val() !== '') {
        if ($('#msg_phone2').text() !== '') {
    ...
    if ($("#country").val() !== '') {
        ...
        $('#msg_country').html("*" + _lang.Required + "*").css('color', 'red');
        count++;
    }
    if ($("#city").val() !== '') {
        if ($("#country").val() == 'LB') {
    ...
    if ($("#place").val() !== '') {
        ...
        $('#msg_place').html("*" + _lang.Required + "*").css('color', 'red');
        count++;
    }
    if ($("#street").val() !== '') {
        $('#msg_street').html('');
        ...
        $('#msg_street').html("*" + _lang.Required + "*").css('color', 'red');
        count++;
    }
    // if ($('#terms').is(":checked")) {
    //     $('#msg_terms').html('');
    // } else {
    //     $('#msg_terms').html("*" + _lang.Required + "*").css('color', 'red');
    //     count++;
    // }
    // if ($("#building").val() !== '') {
    //     $('#msg_building').html('');
    // }
    // else {
    //     $('#msg_building').html("*"+_lang.Required+"*").css('color', 'red');
    //     count++;
    // }
    // if ($("#floor").val() !== '') {
    //     $('#msg_floor').html('');
    // }
    // else {
    //     $('#msg_floor').html("*"+_lang.Required+"*").css('color', 'red');
    //     count++;
    // }
    if (count !== 0) {
        return false;
