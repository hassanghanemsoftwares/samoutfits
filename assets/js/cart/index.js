                            ...
                            if (today < expiry_date) {
                                if (data['limited'] === '1') {
                                    $.ajax({
                                        ...
                                        url: getAppURL('checkout/check_customer_coupon_usage_times'),
...
function calculateTotal() {
    var total = 0;
    $("#cartLines").find('tr').each(function (index) {
...
function validation() {
    ...
    if ($('#loged_in_user').val() == '') {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                'item_ids': item_ids, 'qtys': qtys, 'sizes': sizes
            },
            url: getAppURL('checkout/check_items_qty'),
            success: function (data) {
                $("#cartLines").find('tr').each(function (index) {
                    var item_id = $(this).find("td").find('input.r-item_id').val();
                    if (parseFloat(data[item_id]['av_qty']) >= parseFloat(data[item_id]['qty'])) {
                        $(this).css("background-color", "white");
                        $(this).find('td span.i-av_qty').html('');
                    } else {
                        $(this).css("background-color", "#ffcccb");
                        $(this).find('td span.i-av_qty').html("Available Qty: " + data[item_id]['av_qty']);
                        count1++;
                    }
                });
            }
